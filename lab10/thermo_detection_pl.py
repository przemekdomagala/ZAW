...
############# TODO0 ###############
# Ustaw ścieżki
test_rgb = "neural_network-20250531T092419Z-1-001/neural_network/test_rgb"  # Ścieżka do foldeu test_rgb
test_thermal = "neural_network-20250531T092419Z-1-001/neural_network/test_thermal"  # Ścieżka do foldeu test_thermal
###################################
...
net_rgb = None
if FUSION == "EARLY":
    net_fus = cv2.dnn.readNet('neural_network-20250531T092419Z-1-001/neural_network/yolov3_training_last_f.weights', 'neural_network-20250531T092419Z-1-001/neural_network/yolov3_testing/yolov3_testing_f.cfg')
if FUSION == "LATE":
    net_therm = cv2.dnn.readNet('neural_network-20250531T092419Z-1-001/neural_network/yolov3_training_last_t.weights', 'neural_network-20250531T092419Z-1-001/neural_network/yolov3_testing/yolov3_testing_t.cfg')
    net_rgb = cv2.dnn.readNet('neural_network-20250531T092419Z-1-001/neural_network/yolov3_training_last_c.weights', 'neural_network-20250531T092419Z-1-001/neural_network/yolov3_testing/yolov3_testing_c.cfg')
...
for i in range(200, 300): # można zmieniać range do 518
    ...
    if FUSION == "EARLY":
        ...
        new_fus = np.zeros_like(img_rgb, dtype=np.uint8)
        new_fus[:, :, :2] = img_rgb[:, :, :2].copy()
        new_fus[:, :, 2] = np.maximum(img_rgb[:, :, 2], img_thermal).astype("uint8")
        ############# KONIEC TODO1 ##########
    ...
    if FUSION == "LATE":
        ...
        #1
        boxes_iou = []
        for j, rect1 in enumerate(Rect1):
            for k, rect2 in enumerate(Rect2):
                iou = IoU(rect1, rect2)
                if iou > 0:
                    boxes_iou.append([(j, k), iou])
        #2
        ...
        for pair in paired_boxes:
            r1 = Rect1[pair[0]]
            r2 = Rect2[pair[1]]
            avg_r = [int((r1[i] + r2[i]) / 2) for i in range(4)]
            boxes.append(avg_r) 
        ############# KONIEC TODO2 ##########
